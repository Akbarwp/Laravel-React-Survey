...
class SurveyController extends Controller
{
    ...
    public function store(StoreSurveyRequest $request)
    {
        ...
        // Mengecek jika terdapat gambar dan Menyimpan pada Local file system
        if (isset($data['image'])) {
    ...
    public function update(UpdateSurveyRequest $request, Survey $survey)
    {
        ...
        if (isset($data['image'])) {
            ...
            // Jika terdapat gambar lama, dan akan dihapus
            if ($survey->image) {
        ...
        // Update survey pada Database
        $survey->update($data);

        // Get id sebagai plain array pada pertanyaan yang ada
        $existingId = $survey->questions()->pluck('id')->toArray();
        
        // Get id sebagai plain array pada pertanyaan yang baru
        $newId = Arr::pluck($data['questions'], 'id');

        // Find questions to delete
        $toDelete = array_diff($existingId, $newId);

        // Find question to add
        $toAdd = array_diff($newId, $existingId);
    ...
    private function saveImage($image)
    {
        ...
        if (preg_match('/^data:image\/(\w+);base64,/', $image, $type)) {
            ...
            // Mengambil file extension --> jpg, png, gif
